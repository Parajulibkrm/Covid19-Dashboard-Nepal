...
function colorMap(dataOfDay,option) {
    ...
    var dateEntry = d3.select('svg')
                    ...
                    .attr("x",10)
                    .attr("y",290)
                    .text(dataOfDay[0].data_date.toLocaleString('default', { month: 'long' })+' ' + dataOfDay[0].date_day)
	...
	legend
		.append('text')
		.attr("x", 600 - 60) //leave 5 pixel space after the <rect>
		.attr("y", function(d, i) {
			return i * 10;
		})
		...
		.text(function(d) {
            ...
            if (r[1]) {
                return r[0].toFixed(1)+" to "+ r[1].toFixed(1)
            }
            return "0.0 to 0.0"
		});
}