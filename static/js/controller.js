d3.queue()
  ...
  .defer(d3.csv,"https://raw.githubusercontent.com/Parajulibkrm/covid19-district-data-nepal/master/CoronaNepal.csv", (row)=>({
    districtId: row.ID,
  ...
  .await((error,mapData,coronaData)=> {
    ...
    const setHeaderData= () => {
        ...
        document.getElementById("ld").textContent= maxDate.toLocaleString('default', { month: 'long' })+' '+maxDate.getDate();
    }
    ...
    d3.select('#playButton')
      .on("click",() => {
        if (playing) document.getElementById('playButton').innerHTML = "Pause"
        if (!playing) document.getElementById('playButton').innerHTML = "Play"
          playing = !playing;
          if (playing) return;
            window.setInterval(()=> {
